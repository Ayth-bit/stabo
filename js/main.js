...
map.on("load", function () {
  ...
  map.on("contextmenu", function (e) {
    ...
    if (isDebugMode || distance <= 1) {
      ...
      alert(
        "You can only create threads within a 1km radius of your current location."
      );
    }
  });
});
  ...
  fetch("/api/threads")
    ...
    .then((threads) => {
      threads.forEach((thread) => {
        const distance = turf.distance(
          userLocation,
          [thread.coordinates.lng, thread.coordinates.lat],
          { units: "kilometers" }
        );
        if (isDebugMode || distance <= 1) {
